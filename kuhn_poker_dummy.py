...
_NUM_PLAYERS = 2
_DECK = frozenset([0, 1, 2, 3, 4])
_GAME_TYPE = pyspiel.GameType(
    short_name="kuhn_poker_dummy",
    long_name="Kuhn Poker Dummy",
...
class KuhnPokerState(pyspiel.State):
  ...
  def _legal_actions(self, player):
    ...
    if player == pyspiel.PlayerId.CHANCE:
      ...
      return [Action.PASS, Action.BET, Action.PASS1, Action.BET1]
  ...
  def __str__(self):
    ...
    return "".join([str(c) for c in self.cards] + ["pb"[b%2] for b in self.bets])
...
class KuhnPokerObserver:
  ...
  def __init__(self, iig_obs_type, params):
    ...
    if iig_obs_type.private_info == pyspiel.PrivateInfoType.SINGLE_PLAYER:
      pieces.append(("private_card", 5, (5,)))
    if iig_obs_type.public_info:
      if iig_obs_type.perfect_recall:
        pieces.append(("betting", 12, (3, 4)))
      else:
        pieces.append(("pot_contribution", 4, (4,)))
  ...
  def string_from(self, state, player):
    ...
    if "betting" in self.dict and state.bets:
      pieces.append("".join("pb"[b%2] for b in state.bets))
